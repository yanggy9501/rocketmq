...
public class RouteInfoManager {
    ...
    /**
     * rocketmq 高阶功能，可以基于tag来进行数据筛选，比较简单，没法支持更加复杂
     * rocketmq 是支持一个高阶功能，叫 filter server，可以在每台 broker 上启动一个 filter server
     * filter server 启动之后会根本地的 broker 来进行长连接构建，注册，以及心跳保活
     * 我们可以把一个自定义的消息筛选的class（一个类）上传到 filter server 实现高阶的筛选
     */
    ...
    /**
     * mq broker 启动后会向 namesrv 注册 Broker
     *
     * @param clusterName broker 所属集群
